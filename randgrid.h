...
class RandGrid { // RandGrid generates 4x4 (internally 9x9) puzzle grids.
    ...
    Grid randBlocks(int numBlocks, int numCuts) { // Generates a grid with a single region partitioned into blocks.
        ...
        for (auto i : gridpoints) std::cout << "[" << i.first << " " << i.second << "] ";
        std::cout << "\n";
        ...
        for (auto i : thingsvec) std::cout << "<" << i.first << " " << i.second << "> ";
        std::cout << "\n";
    ...
    void disp() {
        std::cout << possiblePaths.size() << " PATHS FOUND\n";
        std::cout << gridRegions.size() << " REGIONS FOUND\n";
    }
    ...
    void visualize(int x) {
        ...
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                if (possiblePaths[x].find({i, j}) != possiblePaths[x].end()) std::cout << "#";
                else {
                    bool bad = true;
